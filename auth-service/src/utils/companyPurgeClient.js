...
// Order matters: auth-service purges tbl_business itself LAST (see
// business-repo.js's purgeCompany), after every one of these has succeeded —
// so a partial failure never leaves tbl_business gone while other services
// still hold data for it. Stop on first failure; the caller reports which
// service failed so a retry can re-run the whole sequence (every fn_purge_company
...
// Calls POST /internal/companies/:zodu_id/purge on every service in
...
exports.purgeCompanyAcrossServices = async (zodu_id) => {
  const results = [];

  for (const svc of SERVICES) {
    try {
      const res = await axios.post(
        `${svc.baseURL}/internal/companies/${zodu_id}/purge`,
        {},
        { timeout: 30000 }
      );
